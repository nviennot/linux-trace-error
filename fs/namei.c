		...
		bool want_excl)
{
	...
	if (!dir->i_op->create)
		return -ERR(EACCES);	/* shouldn't it be ENOSYS? */
	mode &= S_IALLUGO;
	mode |= S_IFREG;
...
{
	struct dentry *dentry = ERR_PTR(-ERR(EEXIST));
	struct qstr last;
	int type;
	int err2;
	int error;
...
{
	const char *res = ERR_PTR(-ERR(EINVAL));
	struct inode *inode = d_inode(dentry);
