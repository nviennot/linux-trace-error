				 ...
				 union sctp_addr *daddr, int addr_len)
{
	...
	if (old && old != asoc)
		return old->state >= SCTP_STATE_ESTABLISHED ? -ERR(EISCONN)
							    	: -ERR(EALREADY);
			  ...
			  int addrs_size, int flags, sctp_assoc_t *assoc_id)
{
	...
	if (asoc)
		return asoc->state >= SCTP_STATE_ESTABLISHED ? -ERR(EISCONN)
			: -ERR(EALREADY);
